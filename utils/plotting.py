...
def plot_reconstruction(target_load, target_area, pred_load, pred_area, 
                        target_params, pred_params, epoch):
    """
    Creates a 2x2 figure comparing Curves AND Parameters.
    
    Args:
        target_load, target_area: (N_steps,) arrays
        pred_load, pred_area:     (N_steps,) arrays
        target_params: (32,) array [16 exponents, 16 offsets]
        pred_params:   (32,) array
    """
    ...
    axs[0, 0].set_title(f"Load vs Step (Epoch {epoch})")
    axs[0, 0].legend()
    axs[0, 0].grid(True, alpha=0.3)

    # 2. Area vs Step
    axs[0, 1].plot(target_area, label='Target (GT)', color='black', linewidth=2)
    axs[0, 1].plot(pred_area, label='Pred', color='blue', linestyle='--')
    axs[0, 1].set_title("Contact Area vs Step")
    axs[0, 1].legend()
    axs[0, 1].grid(True, alpha=0.3)
    
    # --- Row 2: The Design (Parameters) ---
    
    # Split params (Assuming 16 asperities)
    n_asp = 16
    ...
    axs[1, 0].set_title("Exponents (n) per Asperity")
    axs[1, 0].set_xlabel("Asperity Index (Sorted by Height)")
    axs[1, 0].legend()
