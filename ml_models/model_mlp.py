...
class SurfaceInverseModel(nn.Module):
    def __init__(self, config):
        ...
        # keep channel sizes fixed as they extract shapes, 
        # not related to the number of asperities.
        ...
        # Pass a dummy input to calculate the exact flatten size automatically
        dummy_input = torch.zeros(1, 2, n_steps)
        with torch.no_grad():
            dummy_out = self.conv_layers(dummy_input)
            self.flatten_size = dummy_out.view(1, -1).size(1)
        
        # --- Decoder (Parameter Regressor) ---
        # We scale the hidden layer based on n_asperities to ensure capacity
        ...
        self.fc_layers = nn.Sequential(
            ...
            # Output: 2 values per asperity (Exponent + Height_Offset)
