...
class SurfaceGenerator:
    ...
    def generate_canonical_singles(self, n_samples):
        ...
        """
        print(
            f"  > Generating {n_samples} Canonical Singles (Basis Functions)...")
    ...
    def generate_canonical_walls(self, n_samples):
        ...
        for i in range(n_samples):
            ...
            if mode < 0.5:
                ...
                # Inactive: Far away
                h_inactive = (
                    1.0 + 0.2 * torch.rand(self.n_asp - n_active)) * self.max_delta
    ...
    def generate_multistep(self, n_samples):
        ...
        for i in range(n_samples):
            ...
            if n_levels < self.n_asp:
                ...
                cuts = np.sort(np.random.choice(
                    range(1, self.n_asp), n_levels - 1, replace=False))
            ...
            if combined_h:
                ...
                if len(h_seq) < self.n_asp:
                    padding = torch.ones(
                        self.n_asp - len(h_seq)) * self.max_delta
...
if __name__ == "__main__":
    ...
    for batch_n, batch_h in tqdm(loader, desc="Physics Engine"):
        with torch.no_grad():
            ...
            # Stiffness
            stiffness = torch.diff(
                load, dim=1, prepend=torch.zeros(current_batch, 1).to(device))
